...
- 学生だと無料で private repository いくつでも作れる。
  - 研究室でも同じく private repository が作れる
...
- [Google の colab と google drive は相性がよさそう](https://colab.research.google.com/notebooks/io.ipynb) のもあってgoogle drive を使っている
...
以前は、以下のような感じでしたが、UCL行ってたときに上記の方法に収束しました。
...
- ベクタ形式は拡大してもボケないのと、bounding box の指定をしたくないので PDF形式を使う。
...
- `booktabs` の中身を `pandas` の `to_latex` で生成する。
