...
- FB messanger と workplace: LaTeX が使える。別にこれでもいいと思ってるけど、あんまり聞かない。
- Google meet: 研究室のゼミがこれ。これでいいと思う。同社の Google jamboard (同時に複数人で触れるホワイトボード) が便利
  - Mac の場合、sidecar で iPad を外部ディスプレイとしてつなぎ、Jamboard に Apple Pencil で書き込む
...
- ベクタ形式は拡大してもボケないのと、bounding box の指定をしたくないので PDF 形式を使う。
  ...
  - 追記するメリットとしては、PDF は github で展開されるけど、 EPS は2019年の段階ではされてない
