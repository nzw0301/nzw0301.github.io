...
- Zotero 自体は文献管理ソフト
- pdf ファイルの保存先を Google drive に指定してるので、どの PC / タブレットでも同じようにアクセス可能
- [better bibtex](https://retorque.re/zotero-better-bibtex/) のプラグインをインストールするとBibTeX を自動で生成するので、論文を書くときにこの BibTeX ファイルを参照している
  - ただグローバルな `*.bib` ファイルを生成してるので、無関係な書誌情報も含んでおり共著者に共有しにくい。今の所は不要なエントリを削除するスクリプトを適用してから共有してる
- 類似アプリの Mendeley はアカデミックだと200GBの容量が無料であるのに対して zotero は複数マシン間で同期できる容量が300MBしかなく、それ以上にはお金を払うか自分でストレージを用意する必要があり[^1]、自分は[teracloud](https://teracloud.jp/en/)の無料ストレージを使っています。

5年ほどMendelyを使っていたのですが、MacOS のサポートが微妙（最新の OS に最新の Mendeley がインストールできない、検索のUIがよくない、のもあって移行しました）

## [Notion](https://www.notion.so/)

TODO管理や研究メモ用。LaTeXサポートが嬉しい。あんまり使いこなせてないです。
...
- [zulip](https://zulip.com/): LaTeX が使える。個人的には全部これになってほしいが、検索が弱い。
...
[Notion](https://www.notion.so/)のTODOリストを使っています。
...
- スマホアプリがある
- [google keep](https://keep.google.com/) は付箋みたいな感じでこっちのほうが複雑そうなことができるが、自分はシンプルな google tasks を使った
