...
`(i / SIGMOID_TABLE_SIZE * 2 - 1) * MAX_SIGMOID`は、index`i`を`-MAX_SIGMOID`から`MAX_SIGMOID`を`SIGMOID_TABLE_SIZE-1`に等分割したときの`i`番目の実数値に変換している。当然だが、`sigmoid(x)`の最終行は、これの逆変換になっている。
