...
\begin{aligned}
    ...
    =& \sum_{\boldsymbol{z}} \prod_{k=1}^2 \pi_k^{z_k} \prod_{k=1}^2 N(\boldsymbol{x}|\boldsymbol{\mu_k}, \Sigma_{k})^{z_k} \\
...
一般化して $$K$$ の場合は，
$$ \sum_{\boldsymbol{z}}p(\boldsymbol{z})p(\boldsymbol{x}|\boldsymbol{z}) $$
は，内部の同時確率を変形した式を $$ \prod $$ 展開してから $$ \sum_{\boldsymbol{z}} $$ で足しあわせる．

足し合わせると
$$z_k=1$$ を満たす $$k$$ での
$$ \pi_k N(\boldsymbol{x} \mid \boldsymbol{\mu_k},\Sigma_k) $$ の
添字が $$1$$ から $$ K $$ までの和だけが残るため，式(9.12)のように周辺化することで式(9.7)を導くことができる．
