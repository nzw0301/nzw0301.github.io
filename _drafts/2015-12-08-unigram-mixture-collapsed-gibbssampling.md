...
青いトピックモデル本の混合ユニグラムモデルのベイズ推定（周辺化ギブスサンプ）のPython実装です．本p53のアルゴリズムです．概要は[こちら](https://nzw0301.github.io/2015/12/mixgibbs)
...
`doc id`はデータの行数-1に相当します．`doc id=5`のときにトピックが混じっていることがわかります．一方それ以外のデータについては片方のトピックに偏っています（一方だけ約99%になっている）
