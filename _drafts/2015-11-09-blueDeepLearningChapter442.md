...
前回の続きです．順伝播，誤差逆伝播，重みとバイアスの更新を行列の表記で行います．

### 表記

- $$N$$ ：ミニバッチのデータ数
- $$\boldsymbol{x}_n$$：1件のデータ(例えば1つ文書，行は単語の頻度とか)
- $$\boldsymbol{X} = \left[ \begin{array}{c} \boldsymbol{x}_1 \ldots \boldsymbol{x}_N \end{array} \right]$$：ミニバッチの行列
...
$$
\begin{aligned}
    \boldsymbol{U}^{(l)} &= \boldsymbol{W^{(l)}} \boldsymbol{Z^{(l-1)}} + \boldsymbol{b}^{(l)} \boldsymbol{1}_N^{\top} \\
    \boldsymbol{Z}^{(l)} &= f^{(l)}(\boldsymbol{U}^{(l)})
\end{aligned}
$$
...
出力層のデルタ$$\boldsymbol{\Delta^{(L)}} = \boldsymbol{D} - \boldsymbol{Y}$$ を $$(L-1), \cdots, 2$$ まで逆伝播させる．

$$
\begin{aligned}
    \boldsymbol{\Delta}^{(l)} = f'^{(l)}(\boldsymbol{U}^{(l)}) \odot (\boldsymbol{W}^{(l+1)T} \boldsymbol{\Delta}^{(l+1)})
\end{aligned}
$$
...
逆伝播で計算したデルタを使って微分し，重み$$\boldsymbol{W}$$を更新．誤差逆伝播法で漸化式を求めたので，重みの更新は並列して計算ができる．
...
$$
\begin{aligned}
    \partial \boldsymbol{W}^{(l)} &= \frac{1}{N} \boldsymbol{\Delta^{(l)}} \boldsymbol{Z^{(l-1)T}} \\
    \partial \boldsymbol{b}^{(l)} &= \frac{1}{N} \boldsymbol{\Delta^{(l)}} \boldsymbol{1}_N
\end{aligned}
$$
...
\begin{aligned}
    \boldsymbol{W}^{(l)} & \leftarrow
        \boldsymbol{W}^{(l)} - \epsilon \partial \boldsymbol{W}^{(l)} \\
    \boldsymbol{b}^{(l)} & \leftarrow
        \boldsymbol{b}^{(l)} - \epsilon \partial \boldsymbol{b}^{(l)} \\
\end{aligned}
$$
