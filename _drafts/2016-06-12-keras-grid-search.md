...
分類を行う場合は`KerasClassifier`にKerasのモデルを渡すことで，scikit-learnの分類器と同じように扱えます．このとき，Kerasの`fit()`で使用するパラメータ`nb_epoch, batch_size, verbose`は`KerasClassifier`のコンストラクタに渡すことができます．ちなみにこれらの値もgrid-searchで探索可能です．あとはsklearnと同様，探索するパラメータを辞書型のデータで渡すだけです．
